...
var child;
var fails = 0;
var goBinary = './clicker-game'; //or template.exe
...
function fire_event(name, sender) {
	var msg = {
		...
		inputs: element_by_tag_as_array('input').concat(
			element_by_tag_as_array('select')
		),
	};
...
function avoid_reload() {
	if (sessionStorage.getItem('loaded') == 'true') {
		alert(
			'go-webkit will fail when page reload. avoid using <form> or submit.'
		);
		close();
	}
